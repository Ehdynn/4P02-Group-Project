...
public class StringTiling {
     ...
     */
    public static List<Sequence> tile(Token[] submission, List<Token[]> database, int minLength){
        List<Sequence> matches = new ArrayList<>();
        boolean[] marked = new boolean[submission.length];

        for(Token[] compareTo: database){
            int numMatches = 1;
                ...
                for (int i = 0; i < submission.length; i++) {
                    ...
                    for (int j = 0; j < compareTo.length; j++) {
                        ...
                        while (i + n < submission.length &&
                                j + n < compareTo.length &&
                                !marked[i + n] &&
                                submission[i + n].compareTo(compareTo[j + n]) >= 0) {
                            n++;
                            if(submission[i + n].compareTo(compareTo[j + n]) == 1){
                                //if token has identical type and value, assign full plagiarism value
                                submission[i + n].setPlagiarismValue(1);
                            } else if (submission[i + n].compareTo(compareTo[j + n]) == 0){
